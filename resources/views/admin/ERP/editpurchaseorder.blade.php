											...
											<div class="form-group row" data-select2-id="243" style="margin-bottom:0px !important">
												...
												<div class="col-lg-6 col-md-6 col-sm-6" data-select2-id="242">
													<select class="form-control" id="kt_select2_1" name="seller">
														<option value="" class="dropdown">Select</option>
														@foreach($sellers as $val)
														@if($value -> seller_id == $val -> id)
														<option value="{{$value->seller_id}}" selected>{{$value -> seller_name}}</option>
														@else
										...
										<div class="col-md-4" style="margin:10px" >
											<div class="row">
													...
													<div class="col-lg-6 col-xl-6">
														<input class="form-control form-control-solid form-control-lg" value="{{$value -> invoice_id}}" id="invoice" name="invoice" type="text" />
  ...
  <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" >
					...
					<thead>
						<tr>	
							...
							<th><button class="addRow btn btn-primary">+</button></th>
						</tr>
					</thead>	
					<tbody>
					<?php $sum = 0; ?>
					 ...
					 <?php $x = 0; ?>
						<tr>
							...
							@foreach($mdata->unique('material_name') as $valu)
							 @if( $valu -> id == $item -> material_id )
							 <option value="{{$item -> material_id}}" selected>{{$item -> material_name}}</option>
							 @else
							...
							<td><input type="number" value="{{$item -> material_quantity}}" name="quantity[]" id="quantity" class="form-control quantity" autocomplete="off">
								<span style="color:red;" id="quantity_error" class="field_error"></span></td>
							<td>
								<input type="text" value="{{$item -> type}}" name="type[]" id="type_id" class="form-control" autocomplete="off">
								<span style="color:red;" id="type_error" class="field_error"></span>
							</td>		
              <td><input value="{{$item -> material_price}}" type="number" name="price[]" id="price" class="form-control price" autocomplete="off">
              	<span style="color:red;" id="price_error" class="field_error"></span></td>
							<td><input value="{{$item -> material_quantity * $item -> material_price}}" type="number" name="total[]" id="total" class="form-control total" autocomplete="off">
								<span style="color:red;" id="total_error" class="field_error"></span></td>
							<td><button class="btn btn-danger remove" style="">-</button></td>
						</tr>
						@endforeach
					</tbody>
	...
	<div class="row">
			...
			<div class="form-group row">
				...
				<label>Upload Invoice</label>
					<div class="col-lg-9">
						...
						<p><a href="/../upload/{{$value->invoice}}">View Invoice</a></p>
					</div>
			</div>
			<br>
			<div class="form-group row">
					<label>Comment</label>
						<input class="form-control form-control-solid form-control-lg" name="comment" id="comment" type="text" value="{{$value->comment}}" />
		...
		<select name="status" id="status" class="form-control">
			...
			@elseif($value -> order_status == 'Accept')
			...
			<option value="Recieved">Recieved</option>
			@elseif($value -> order_status == 'Recieved')
			<option value="Pending">Pending</option>
			<option value="Accept" selected>Accept</option>
			<option value="Reject">Reject</option>
		...
		<div class="col-md-2" style="margin:10px;">
			<div class="form-group row">
				<label>Sub-Total : </label>
					<b id="subtotal" style="margin-left:20px;">{{$value->total}}</b>
				</input>
			</div>
			<div class="form-group row">
					<label>Discount</label>
						<input class="form-control form-control-solid form-control-lg" name="discount" id="discount" type="text" value="{{$value->discount}}" />
...
<script>
  ...
  <script>
    ...
    function addRow()
    {
		...
		htmlRows += '<td><button type="button" class="btn btn-danger remove" onclick="remove()">-</button></td>';
		htmlRows += '</tr>';
